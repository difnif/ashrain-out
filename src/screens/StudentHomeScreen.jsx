...
function ArchiveTab({ theme, archive, setArchive, playSfx }) {
  ...
  const toggleHidden = (id) => setArchive(prev => prev.map(a => a.id === id ? { ...a, hidden: !a.hidden } : a));
  const deleteItem = (id) => setArchive(prev => prev.filter(a => a.id !== id));
    ...
    <div style={{ padding: "12px 16px", animation: "fadeIn 0.3s ease" }}>
      ...
      {filtered.map(item => (
        <div key={item.id} style={{
          ...
          background: item.hidden ? `${theme.card}80` : theme.card,
          border: `1px solid ${item.hidden ? theme.border + "60" : theme.border}`,
          opacity: item.hidden && filter !== "hidden" ? 0.6 : 1,
        }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: theme.text, marginBottom: 4 }}>{item.title}</div>
          {item.preview && <div style={{ fontSize: 11, color: theme.textSec, marginBottom: 6, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.preview}</div>}
          <div style={{ fontSize: 10, color: theme.textSec, marginBottom: 6 }}>{dateStr(item.createdAt)} · {item.type || "학습"}</div>
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => { togglePublic(item.id); playSfx("click"); }}
              style={{ fontSize: 9, padding: "3px 8px", borderRadius: 6, border: `1px solid ${theme.border}`, background: item.isPublic ? `${PASTEL.mint}10` : theme.card, color: item.isPublic ? PASTEL.mint : theme.textSec, cursor: "pointer" }}>
              {item.isPublic ? "🌍 공개" : "🔒 비공개"}
            </button>
            <button onClick={() => { toggleHidden(item.id); playSfx("click"); }}
              style={{ fontSize: 9, padding: "3px 8px", borderRadius: 6, border: `1px solid ${theme.border}`, background: theme.card, color: theme.textSec, cursor: "pointer" }}>
              {item.hidden ? "📂 피드로" : "👁️‍🗨️ 숨기기"}
            </button>
            <button onClick={() => { if (confirm("삭제하면 학습량에서도 제외돼요. 숨기기를 사용할까요?")) deleteItem(item.id); }}
              style={{ fontSize: 9, padding: "3px 8px", borderRadius: 6, border: `1px solid ${PASTEL.coral}30`, background: "transparent", color: PASTEL.coral, cursor: "pointer" }}>
              🗑️
            </button>
          </div>
        </div>
    ...
    <div style={{ padding: "12px 16px", animation: "fadeIn 0.3s ease" }}>
      {pending.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          ...
          {pending.map(hw => {
            const isEx = expandedId === hw.id;
            return (
              <div key={hw.id} style={{ marginBottom: 8, borderRadius: 14, overflow: "hidden", border: `2px solid ${PASTEL.coral}40`, background: `${PASTEL.coral}04` }}>
                <button onClick={() => { setExpandedId(isEx ? null : hw.id); setAnswerText(""); }}
                  style={{ width: "100%", padding: "12px 14px", border: "none", background: "transparent", cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between" }}>
                  <div>
                  ...
                  <span style={{ fontSize: 11, color: theme.textSec }}>{isEx ? "▲" : "▼"}</span>
                </button>
                {isEx && (
                  <div style={{ padding: "0 14px 14px", animation: "fadeIn 0.2s ease" }}>
                    <div style={{ padding: 12, borderRadius: 10, background: theme.bg, marginBottom: 10, fontSize: 13, lineHeight: 2, color: theme.text }}>{hw.problemText || "문제 확인"}</div>
                    {hw.teacherResponse && (
                    ...
                    <textarea value={answerText} onChange={e => setAnswerText(e.target.value)} placeholder="답을 적어주세요..."
                      style={{ width: "100%", padding: 10, borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bg, color: theme.text, fontSize: 13, minHeight: 80, resize: "vertical", boxSizing: "border-box" }} />
                    <button onClick={() => {
                      if (!answerText.trim()) { showMsg("답을 적어주세요!", 1500); return; }
                      setHomework(prev => prev.map(h => h.id === hw.id ? { ...h, status: "completed", completedAt: Date.now(), answer: answerText } : h));
                      setExpandedId(null); playSfx("success"); showMsg("숙제 완료!", 1500);
                    }} style={{ width: "100%", marginTop: 8, padding: 12, borderRadius: 12, border: "none", background: PASTEL.coral, color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>✅ 숙제 제출</button>
                  </div>
                )}
              </div>
            );
