    ...
    const animate = (timestamp) => {
      ...
      if (progress < 1) {
        ...
        if (drawGoal === "combined") {
          ...
          // Widen viewBox for side-by-side comparison
          if (triangle) {
            const {A,B,C} = triangle;
            ...
            const cloneShift = triW + gap;
            // Bounds: original+circumcircle on left, clone on right
            const minX = Math.min(A.x,B.x,C.x, circumO?circumO.x-R:0) - 20;
            const maxX = Math.max(A.x,B.x,C.x) + cloneShift + triW + 20;
            const minY = Math.min(A.y,B.y,C.y, circumO?circumO.y-R:0) - 30;
            const maxY = Math.max(A.y,B.y,C.y, circumO?circumO.y+R:0) + 20;
